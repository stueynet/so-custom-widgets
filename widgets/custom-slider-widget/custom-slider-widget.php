<?php
/*
Widget Name: Custom Slider widget
Description: A very simple slider widget modified to have a background color.
Author: Stuart Starr
Author URI: http://stuey.net
*/

class Custom_Slider_Widget extends SiteOrigin_Widget {
	function __construct() {
		parent::__construct(
			'custom-slider',
			__('Custom Slider', 'siteorigin-widgets'),
			array(
	...
	function initialize() {
		...
		$frontend_scripts[] = array(
			'sow-slider-slider-cycle2',
                plugin_dir_url(__FILE__) . 'js/jquery.cycle' . SOW_BUNDLE_JS_SUFFIX . '.js',
			array( 'jquery' ),
			SOW_BUNDLE_VERSION
		);
		if( function_exists('wp_is_mobile') && wp_is_mobile() ) {
			$frontend_scripts[] = array(
				'sow-slider-slider-cycle2-swipe',
                    plugin_dir_url(__FILE__) . 'js/jquery.cycle.swipe' . SOW_BUNDLE_JS_SUFFIX . '.js',
				array( 'jquery' ),
				SOW_BUNDLE_VERSION
			);
		}
		$frontend_scripts[] = array(
			'sow-slider-slider',
                plugin_dir_url(__FILE__) . 'js/slider' . SOW_BUNDLE_JS_SUFFIX . '.js',
			array( 'jquery' ),
			SOW_BUNDLE_VERSION
		);
...
siteorigin_widget_register('custom-slider', __FILE__, 'Custom_Slider_Widget');
