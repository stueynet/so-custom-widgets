	...
	<ul class="sow-slider-images" data-settings="<?php echo esc_attr(json_encode($settings)) ?>">
		...
		foreach($instance['frames'] as $frame) {
			...
			<li
				...
				style="<?php if(!empty($background_image[0]) && (!empty($foreground_image)) || !empty($frame['background_videos']) ) echo 'background-image: url('.$background_image[0].');' ?>; background-color: <?php echo $frame['background_color']; ?>">
				<?php
